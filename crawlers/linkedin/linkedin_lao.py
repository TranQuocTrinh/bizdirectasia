...
number_search = 1000
pathFile = "lao.csv"
pathFolder = "./data_lao"+str(stt)+"/"
pathResult = "./data_lao"+str(stt)+"/results.csv"
...
class LinkedIn:
    ...
    def search(self, company_key, company_name):
        search = "laos & intitle:" + '"- '+ company_name+ '"'+'& "linkedin.com/in/"'
        print("search:", search)
...
def get_ips(one=1):
    if one == 0:
        requests.get("https://places.bizdirectasia.com/proxy/renew?group=2")
        time.sleep(600)
    driver = requests.get("https://places.bizdirectasia.com/proxy/list?group=2")
