...
stt = 0
number_search = 1000
pathFile = "myanmar.csv"
pathFolder = "./data_myanmar"+str(stt)+"/"
pathResult = "./data_myanmar"+str(stt)+"/results.csv"
...
class LinkedIn:
    ...
    def search(self, company_key, company_name):
        search = "myanmar & intitle:" + '"- '+ company_name+ '"'+'& "linkedin.com/in/"'
        print("search:", search)
...
def get_ips(one=1):
    if one == 0:
        requests.get("https://places.bizdirectasia.com/proxy/renew?group=3")
        time.sleep(600)
    driver = requests.get("https://places.bizdirectasia.com/proxy/list?group=3")
