                ...
                for div in divs:
                    try:
                        if div.get_text()[0] != " ":
                            ...
                            print("people_key =", url.get_text().split("com/in/")[1].split("?")[0])
                            value["people_key"] = url.get_text().split("com/in/")[1].split("?")[0]
                            try:
...
def get_ips(one=1):
    ...
    if stt == 4:
        get_list = "https://places.bizdirectasia.com/biz-proxy/list"   
        get_renew = "https://places.bizdirectasia.com/biz-proxy/renew" 
    else:
        get_list = "https://places.bizdirectasia.com/proxy/list?group="+str(stt)
        get_renew = "https://places.bizdirectasia.com/proxy/renew?group="+str(stt)
    
    if one == 0:
        requests.get(get_renew)
        time.sleep(600)
    driver = requests.get(get_list)

    temp = driver.text.split(",")
    ips = []
    for x in temp:
        x = x.strip("\n").lstrip("[").rstrip("]").strip('"')+":3128"
        ips.append(x)
    dct_ips = {}
    for ip in ips:
        dct_ips[ip] = 0
...
def pick_ip(dct_ips):
    if bool(dct_ips) == False:
        return False
    else:
        dct_ips = sorted(dct_ips.items(), key=operator.itemgetter(1))
        if dct_ips[0][1] == 0:
            return dct_ips[0][0]
        else:
            return False
...
def update(dct_ips, current_ip):
    dct_ips.pop(current_ip,None)
    return dct_ips
...
def get_data(skip):
    ...
    while skip <= skip_max:
        ...
        current_ip = pick_ip(dct_ips)
        if current_ip == False:
            ips, dct_ips = get_ips(0)
            print("ip = ", ips)
        else:
            link = LinkedIn(current_ip)
        ...
        for (company_key, company_name) in zip(company_keys, company_names):
            ...
            if data == -1:
                while data == -1:
                    dct_ips = update(dct_ips,current_ip)
                    while pick_ip(dct_ips) == False:
                        ips, dct_ips = get_ips(0)
                        print("ip = ",ips)
                    current_ip = pick_ip(dct_ips)
                    link = LinkedIn(current_ip)
            ...
            if data == False:
                ...
                dct_ips[current_ip] += 1
                results += 1
                profile += len(data["results"])
                datas.append(data)
                print("skip =",d, "\ttime = ",time.time()- start_time)
