...
class SW:
    ...
    def search(self, company_name, company_id, company_key):
        search = company_name
        css_selector = '#viewport > header > div > form > div > input'
        css_sel_click = '#viewport > header > div > form > div > button > svg'
        self.driver.find_element_by_css_selector(css_sel_click).click()
    ...
    def get_detail(self):
        soup = BeautifulSoup(self.driver.page_source, 'lxml').find_all('article', class_='item item--web')
        results = []
        for i,x in enumerate(soup):
            temp = {}
            temp['title'] = x.find_all('h2')[0].get_text().strip()
            temp['url'] = x.find_all('a')[0]['href']
            temp['content'] = x.find_all('p', class_='description')[0].get_text().strip()
            temp['order'] = str(i)
            results.append(temp)
        return results


url = 'https://swisscows.ch/web?query=abc'
# ob = CocCoc(url)
