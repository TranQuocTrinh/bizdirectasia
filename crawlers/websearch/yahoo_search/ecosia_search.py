...
class SW:
    ...
    def search(self, company_name, company_id, company_key):
        ...
        css_selector = 'body > div.results-page > nav > div.container > div > div.col-lg-7.col-sm-12 > div > form > div.search-form-wrapper > input'
    ...
    def get_detail(self):
        try:
            soup = BeautifulSoup(self.driver.page_source, 'lxml').find_all('div', class_='card-desktop card-web')[0].find_all('div', class_='result js-result card-mobile')
        except:
            return []
        results = []
        for i,x in enumerate(soup):
            temp = {}
            try:
                temp['title'] = x.find_all('a')[0].get_text().strip()
                temp['url'] = x.find_all('a')[0]['href']
                temp['content'] = x.find_all('p', class_='result-snippet')[0].get_text().strip()
                temp['order'] = str(i)
                results.append(temp)
            except:
                pass
...
url = 'https://www.ecosia.org/search?q=bizdirect+asia'
