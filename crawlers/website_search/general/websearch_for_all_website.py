import pandas as pd
import json
from bs4 import BeautifulSoup
from tqdm import tqdm
from selenium import webdriver
from selenium.webdriver.chrome.options import Options
from selenium.webdriver.common.desired_capabilities import DesiredCapabilities
import sys
import time
import os
import ast
import requests
import random

if not os.path.exists("./data_website/"):
    os.makedirs("./data_website/")
if not os.path.exists("./data_website/"+sys.argv[1].split(".csv")[0].split("_")[-1]):
    os.makedirs("./data_website/"+sys.argv[1].split(".csv")[0].split("_")[-1])

"""
def pick_proxy():
    ips = []
    for i in range(2):
        url_get_i = "http://68.183.229.8:9069/proxy/list?group="+str(i+1) 
        url_renew_i = "http://68.183.229.8:9069/proxy/renew?group="+str(i+1)
        r = requests.get(url_get_i)
        ips_gr = ast.literal_eval(r.text)
        ips = ips + [ip + ":3128" for ip in ips_gr]

    return random.choice(ips)
"""
def pick_proxy(use_proxy):
    proxies = ['54.255.246.65:3128',
             '54.179.18.192:3128',
              '13.229.214.204:3128',
               '54.179.70.164:3128',
                '54.251.9.143:3128',
                 '13.212.193.102:3128',
                  '13.229.88.223:3128',
                   '54.169.104.46:3128',
                    '13.212.195.230:3128',
                     '54.151.247.120:3128',
                      '52.77.242.160:3128',
                       '122.248.193.131:3128',
                        '13.229.219.204:3128',
                         '13.228.27.18:3128',
                          '13.212.27.218:3128',
                           '13.212.34.203:3128',
                            '13.212.185.15:3128',
                             '54.255.246.39:3128',
                              '13.212.62.187:3128',
                               '13.228.24.203:3128']
    if use_proxy:
        print("Use proxies")
        return random.choice(proxies)
    else:
        return None

# =================================================================== DEFAULT SETTING ==================================
# ================== PROXY and URL ==================

TASK = "yahoo"
USE_PROXY = False
HIDDEN_WINDOW = True


DCT_URL = {
    "zapmeta": "https://www.zapmeta.com.vn/",
    "naver":  "https://www.naver.com/",
    "yahoo": "https://search.yahoo.com/",
    "bing": "https://www.bing.com/",
}
URL = DCT_URL[TASK]
DCT_SELECTOR = {
    "zapmeta": ["#id_search_query", "#id_search_query"],
    "naver": ["#query", "#nx_query"],
    "yahoo": ["#yschsp", "#yschsp"],
    "bing": ["#sb_form_q", "#sb_form_q"],
}

CSS_SELECTOR_SEARCH1 = DCT_SELECTOR[TASK][0]
CSS_SELECTOR_SEARCH2 = DCT_SELECTOR[TASK][1]

print("Search base on {}".format(TASK))
#IP = pick_proxy(USE_PROXY)
# ======================================================================================================================

def extract_detail(soup):
    if TASK == "zapmeta":
        return extract_detail_zapmeta(soup)
    elif TASK == "naver":
        return extract_detail_naver(soup)
    elif TASK == "yahoo":
        return extract_detail_yahoo(soup)
    elif TASK == "bing":
        return extract_detail_bing(soup)

def extract_detail_bing(soup):
    try:
        sub_soup = soup.findAll("h2")
    except:
        return []

    data = []
    i = 0
    for x in sub_soup:
        try:
            temp = {}
            temp["title"] = x.getText(strip=True)
            temp["url"] = x.find("a")["href"]
            temp["_index"] = i
            data.append(temp)
            i += 1
        except:
            pass
    return data

def extract_detail_zapmeta(soup):
    try:
        sub_soup = soup.findAll("h3")
    except:
        return []

    data = []
    i = 0
    for x in sub_soup:
        try:
            temp = {}
            temp["title"] = x.find("a").getText(strip=True)
            temp["url"] = x.find("a")["href"]
            temp["_index"] = i
            if temp["url"].split("/")[0] in {"http:", "https:"}:
                data.append(temp)
                i += 1
        except:
            pass
    return data

def extract_detail_naver(soup):
    try:
        sub_soup = soup.findAll("li", class_="sh_web_top")
    except:
        return []

    data = []
    i = 0
    for x in sub_soup:
        try:
            temp = {}
            temp["title"] = x.find("a").getText(strip=True)
            temp["url"] = x.find("a")["href"]
            temp["_index"] = i
            if temp["url"].split("/")[0] in {"http:", "https:"} and temp["url"].find("naver.com") == -1:
                data.append(temp)
                i += 1
        except:
            pass
    return data

def extract_detail_yahoo(soup):
    try:
        sub_soup = soup.findAll("h3", class_="title ov-h")
    except:
        return []

    data = []
    i = 0
    for x in sub_soup:
        try:
            temp = {}
            temp["title"] = x.find("a").getText(strip=True)
            temp["url"] = x.find("a")["href"]
            temp["_index"] = i
            data.append(temp)
            i += 1
        except:
            pass
    return data

def create_driver(address=None, url=URL, hidden_window=HIDDEN_WINDOW):
    #  Set hiden chrome window: just in case chromedriver 
    chrome_options = Options()
    chrome_options.add_argument("--headless")
    chrome_options.add_argument("--no-sandbox")
    chrome_options.add_argument("--disable-dev-shm-usage")

    #  Set proxy
    if address != None:
        print("IP = {}".format(address))
        proxy = {"address":address}
        capabilities = dict(DesiredCapabilities.CHROME)
        capabilities["proxy"] = {"proxyType": "MANUAL",
                                "httpProxy": proxy["address"],
                                "ftpProxy": proxy["address"],
                                "sslProxy": proxy["address"],
                                "noProxy": "",
                                "class": "org.openqa.selenium.Proxy",
                                "autodetect": False} 

        if hidden_window:
            # driver = webdriver.PhantomJS(desired_capabilities=capabilities)
            driver = webdriver.Chrome(chrome_options=chrome_options, desired_capabilities=capabilities)
        else:
            driver = webdriver.Chrome(desired_capabilities=capabilities)
    else:
        if hidden_window:
            # driver = webdriver.PhantomJS()
            driver = webdriver.Chrome(chrome_options=chrome_options)
        else:
            driver = webdriver.Chrome()
    driver.get(url)
    return driver


"""
def multi_page(driver):
    results = []
    results = results + extract_detail(BeautifulSoup(driver.page_source, "html.parser"))
    for i in range(2, 11):
        try:
            driver.find_element_by_css_selector("#___gcse_0 > div > div > div > div.gsc-wrapper > div.gsc-resultsbox-visible > div.gsc-resultsRoot.gsc-tabData.gsc-tabdActive > div > div.gsc-cursor-box.gs-bidi-start-align > div > div:nth-child("+str(i)+")").click()
            time.sleep(0.5)
        except:
            break
        len_temp = len(results)
        results = results + extract_detail(BeautifulSoup(driver.page_source, "html.parser"))
        if len(results) == len_temp:
            break
    return results
"""

def search(driver, company_name, company_id, company_key, country):
    search = company_name + " " + country + "\n"

    if URL == "https://www.naver.com/":
        url = "https://search.naver.com/search.naver?where=webkr&sm=tab_jum&query=" + search.replace(" ", "+")
        driver.get(url)
        flag_search = True
    else:
        flag_search = False
        for i in range(20):
            try:
                driver.find_element_by_css_selector(CSS_SELECTOR_SEARCH1).clear()
                driver.find_element_by_css_selector(CSS_SELECTOR_SEARCH1).send_keys(search)
                flag_search = True
                break
            except:
                pass

        if flag_search == False:
            for i in range(20):
                try:
                    driver.find_element_by_css_selector(CSS_SELECTOR_SEARCH2).clear()
                    driver.find_element_by_css_selector(CSS_SELECTOR_SEARCH2).send_keys(search)
                    flag_search = True
                    break
                except:
                    pass


    if flag_search:
        time.sleep(0.5)
        soup = BeautifulSoup(driver.page_source, "html.parser")
        # results = multi_page()
        results = extract_detail(soup)
        data = {"company_id":company_id, "company_key": company_key, \
                "company_name": company_name, "results":results}

        return data, len(results), True
    else:
        return {}, 0, False


def main():
    """
    sys.argv[1]:    data website country (csv)
    sys.argv[2]:    end
    sys.argv[3]:    begin
    ##################### python  file_data   number_of_proxy_group   end   begin #####################
    """
    # read_csv
    file_data = sys.argv[1]
    country = file_data.split(".csv")[0].split("_")[-1]
    df = pd.read_csv(file_data)

    end = int(sys.argv[2])
    begin = int(sys.argv[3])
    
    df = df[begin:end].reset_index(drop=True)

    #create driver
    driver = create_driver(address=pick_proxy(USE_PROXY))

    count_block = 0
    count_requests = 1  # 100 requests/proxy
    # for i in tqdm(range(len(df.company_id))):
    i = 0
    while i < len(df)-1:
        i += 1
        try:
            cp_id, cp_key, cp_name = str(df.company_id[i]), str(df.company_key[i]), \
                                        str(df.company_name[i])
        except:
            cp_id, cp_key, cp_name = str(df.company_id[i]), str(df.company_key[i]), \
                                        str(sys.argv[1]).split(".")[-2].split("_")[-1]
        # if i < begin or i > end:    # begin <= i <= end
        #     continue

        start_time = time.time()
        data, num_results, flag = search(driver, cp_name, cp_id, cp_key, country)
        end_time = time.time()
        if flag == False:
            print("driver is false")
            driver.close()
            driver = create_driver(address=pick_proxy(USE_PROXY))
            count_block += 1
            continue

        if num_results != 0 : #Co ket qua -> dump file
            file_name = "./data_website/"+ country +"/"+ str(begin+i)+"_"+cp_id.replace("/","-")+".json"
            json.dump(data, open(file_name,"w"))
            count_block = 0
            print(data["results"])
            line = "end= {},  begin= {},  remainder= {},  percent= {}%,  time= {}|".format(end, begin+i, end-i-begin-1,round(i*100./(end-begin-1),2), round(end_time-start_time,2))
            print("-"*len(line))
            print(line)
            print("-"*len(line))
        else:
            # print("no results")
            count_block += 1
            if count_block > 5000:
                print("--------------BLOCK :)-------------\n--------------BLOCK :)-------------")
                time.sleep(600)
                i = i -5000
    driver.close()

if __name__ == "__main__":
    main()
