...
ips = requests.get('http://68.183.229.8:9069/proxy/list?group=1').text
ips = [ip + ':3128' for ip in ast.literal_eval(ips)]
address = random.choice(ips)

def create_driver(url='https://search.goo.ne.jp/web.jsp?MT=abc&mode=0&sbd=goo001&IE=UTF-8&OE=UTF-8&from=s_b_top_web&PT=TOP'):
    # set hiden chrome
...
def search(driver, company_name, company_id, company_key, country):
    ...
    for i in range(20):
        # try:
        css_selector0 = '#container > div.w645.clearfix.box-srarchBox > form > div.srcBx-inputSearch.srcBx-parent > input.srcBx-mainInput'
        #driver.find_element_by_css_selector(css_selector).clear()
        css_selector = '#header > form > div.srcBx-inputSearch.srcBx-parent > div.srcBx-sb-clear-btn > i'
        driver.find_element_by_css_selector(css_selector).click()
        driver.find_element_by_css_selector(css_selector0).send_keys(search)
        flag_search = True
        #     break
        # except:
        #     pass
    ...
    if flag_search == False:
        for i in range(20):
            # try:
            css_selector = '#header > form > div.srcBx-inputSearch.srcBx-parent > input.srcBx-mainInput'
            driver.find_element_by_css_selector(css_selector).clear()
