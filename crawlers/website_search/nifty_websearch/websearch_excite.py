...
def create_driver(url='https://www.excite.co.jp/'):
    # set hiden chrome
...
def extract_detail(soup):
    try:
        sub_soup = soup.findAll('h3', class_='item_title')
    except:
        return []
...
def search(driver, company_name, company_id, company_key, country):
    ...
    if flag_search == False:
        for i in range(20):
            try:
                css_selector = '#exHeader > div > div.search_box > form > div.searchInput.searchInput-head > input'
                driver.find_element_by_css_selector(css_selector).clear()
                driver.find_element_by_css_selector(css_selector).send_keys(search)
                flag_search = True
                break
