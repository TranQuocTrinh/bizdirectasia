...
def inference(tokenizer, model, lst_keyword, config, device):
    test_dataset = pd.DataFrame({
        "keyword": ["keyword: " + keyword for keyword in lst_keyword]
    })
    ...
    predictions = [" ".join(p.split())
                   if p.split()[0].strip() not in {":", "wiki", "wiki:"}
                   else " ".join(p.split()[1:])
...
@app.post("/keywords2blog")
async def keywords2blog(item: Feature):
    try:
        lst_blog = inference(
            tokenizer=tokenizer,
            model=model,
            lst_keyword=item.lst_keyword,
            config=config,
            device=device
        )
        result = {
            "result": [
                {"keyword": k, "blog": b} for k,b in zip(item.lst_keyword, lst_blog)
            ],
            "status": "OK"
        }
